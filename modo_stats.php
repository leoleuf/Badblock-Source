                ...
                <div class="content">
                    <div class="container-fluid">
						...
						<?php
							...
							else
							{
								...
								if (isset($_GET['username']))
								{
									...
									$data = mysqli_fetch_assoc(mysqli_query($db, "SELECT mcname, rank FROM users WHERE mcname = '".$username."'"));
									
									if (!canOverride($account, $data))
									{
										noEnoughPermissions();
									}
									else
						...
						<div class="row">
									<div class="col-sm-12">
										<div class="card-box">
											<h4 class="m-t-0 header-title"><b>Nombre de sanctions par jour de <?php echo $username; ?></b></h4>
												...
												Highcharts.chart('container', {
													...
													plotOptions: {
														...
														<?php
															...
															$query = mysqli_query($db, "SELECT type, timestamp FROM sanctions WHERE banner = '".secure($db, $username)."' AND timestamp >= '".($time * 1000)."'");
									...
									<div class="col-sm-12">
										<div class="card-box">
											<h4 class="m-t-0 header-title"><b>Objectif du mois de <?php echo $username; ?></b></h4>
											...
											<?php
												...
												$minConnectionTime = getPermission($groups, $userRank, "section.moderation.minConnectionTime", 1);
												$minModTime = getPermission($groups, $userRank, "section.moderation.minModTime", 1);
												$minIGSanction = getPermission($groups, $userRank, "section.moderation.minIGSanction", 1);
												$minGuardianer = getPermission($groups, $userRank, "section.moderation.minGuardianer", 1);
												$minRepartitions = getPermission($groups, $userRank, "section.moderation.minRepartitions", 1);
												
												$minTime = strtotime('first day of this month') * 1000;
												$timeInfo = mysqli_fetch_assoc(mysqli_query($db, "SELECT SUM(totalTime) AS totalTime, SUM(sanctionsTime) AS sanctionsTime, SUM(sanctions) AS sanctions FROM staffSessions WHERE timestamp >= '".$minTime."' && playerName = '".secure($db, $username)."'"));
												...
												$guardianerInfo = mysqli_fetch_assoc(mysqli_query($db, "SELECT COUNT(id) AS total FROM reportMsg WHERE timestamp >= '".$minTime."' && playerTo = '".secure($db, $username)."' && done = 'true'"));
						...
						?>
							<div class="row">
										<div class="col-sm-12">
											<div class="card-box">
												<h4 class="m-t-0 header-title"><b>Rechercher les statistiques d'un joueur</b></h4>

												<form action="modo_stats.php" method="get">
													<div class="row">
															<div class="p-20">
