...
import fr.badblock.bukkit.games.uhc.meetup.runnables.TeleportRunnable;
...
public class UHCScoreboard extends BadblockScoreboardGenerator {
	...
	public UHCScoreboard(BadblockPlayer player){
		...
		this.objective = GameAPI.getAPI().buildCustomObjective("meetup" + rand);
		this.player    = player;

		objective.showObjective(player);
		objective.setDisplayName("§l§6» §b§lUHC MeetUp §l§6«");
	...
	public void generate()
	{
		...
		else if (inGame())
		{
			...
			objective.changeLine(i, "§6│  ");
			i--;
			objective.changeLine(i, "§6├§7 Joueurs en vie: §f" + alivePlayers() + "/" + fr.badblock.bukkit.games.uhc.meetup.runnables.StartRunnable.PLAYERS_ON_START);
			i--;
