...
public class TowerScoreboard extends BadblockScoreboardGenerator {
	...
	public void generate(){
		...
		for(BadblockTeam team : GameAPI.getAPI().getTeams()){
			...
			{
				prefix += ColorConverter.dyeToChat(team.getDyeColor()) + "Â§lâž” ";
			}
			objective.changeLine(i, prefix + team.getChatName().getAsLine(player) + "Â§d(" + team.getOnlinePlayers().size() + ") Â§8> Â§b" + data.getMarks());
			i--;
			teams++;
		}*/
		...
		i--;
		objective.changeLine(i, "§6Â§b§l Equipe: " + teamName);
		i--;
		if (StartRunnable.gameTask != null)
		{
