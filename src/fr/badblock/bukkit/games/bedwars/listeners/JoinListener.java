...
public class JoinListener extends BadListener {
	...
	public void onJoin(PlayerJoinEvent e) {
		...
		if (!player.getBadblockMode().equals(BadblockMode.SPECTATOR) && !inGame()) {
			...
			GameMessages.joinMessage(GameAPI.getGameName(), player.getTabGroupPrefix().getAsLine(player) + player.getName(), Bukkit.getOnlinePlayers().size(), PluginBedWars.getInstance().getMaxPlayers()).broadcast();
		}
	}
	...
	public void onLogin(PlayerLoginEvent e){
		...
		PluginBedWars bedwars = PluginBedWars.getInstance();
		if (Bukkit.getOnlinePlayers().size() + 1 >= bedwars.getMaxPlayers()) {
			if (bedwars.getConfiguration().enabledAutoTeamManager) {
				int max = bedwars.getConfiguration().maxPlayersAutoTeam * bedwars.getAPI().getTeams().size();
				if (bedwars.getMaxPlayers() < max) {
					bedwars.getAPI().getTeams().forEach(team -> team.setMaxPlayers(team.getMaxPlayers() + 1));
					bedwars.setMaxPlayers(bedwars.getMaxPlayers() + bedwars.getAPI().getTeams().size());
					try {
						BukkitUtils.setMaxPlayers(bedwars.getMaxPlayers());
					} catch (Exception err) {
						err.printStackTrace();
					}
				}
		...
		StartRunnable.joinNotify(Bukkit.getOnlinePlayers().size(), Bukkit.getMaxPlayers());
	}
