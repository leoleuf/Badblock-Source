...
public class SheepListener extends BadListener {
	...
	public void onMove(PlayerMoveEvent e){
		if(beforeGame() && e.getPlayer().getVehicle() != null){
			...
			if(b.getType() != Material.AIR && b.getType().isSolid()){
				if(b.getType() == Material.STONE || b.getType() == Material.COBBLESTONE || b.getType() == Material.QUARTZ_BLOCK || b.getType() == Material.OBSIDIAN || b.getType() == Material.COBBLESTONE) b.setType(Material.STAINED_CLAY);
				else if(b.getType() == Material.GRASS || b.getType() == Material.DIRT || b.getType() == Material.NETHER_BRICK || b.getType() == Material.SANDSTONE) b.setType(Material.STAINED_GLASS);
	...
	public void onPvE(EntityDamageByEntityEvent e){
		...
		if(e.getEntityType() == EntityType.SHEEP && e.getDamager().getType() == EntityType.PLAYER){
			...
			Vector vector = new Vector(e.getEntity().getLocation().getX() - e.getDamager().getLocation().getX(), 0, e.getEntity().getLocation().getZ() - e.getDamager().getLocation().getZ());
			...
			if(player.getTeam() != null) sheep.setColor(player.getTeam().getDyeColor());
			else {
