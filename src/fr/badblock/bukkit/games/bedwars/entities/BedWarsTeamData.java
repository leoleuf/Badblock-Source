...
public class BedWarsTeamData implements TeamData {
	@Getter
    private Location firstBedPart, secondBedPart;
	...
	@Getter@Setter
    private TranslatableString bed = new TranslatableString("bedwars.result.teams.bed-keeped");
	...
	public Location findOtherBedPart(){
		...
		if(secondBedPart != null) return secondBedPart;
		Location bed = firstBedPart.clone();
 		Location testLoc = bed.clone().add(1, 0, 0);
		if(testLoc.getBlock().getType() == Material.BED_BLOCK) return testLoc;
		testLoc = bed.clone().add(-1, 0, 0);
		if(testLoc.getBlock().getType() == Material.BED_BLOCK) return secondBedPart = testLoc;
