...
import fr.badblock.bukkit.games.bedwars.inventories.utils.JsonFile;
...
public class InventoriesLoader {
	...
	public static void loadInventories(Plugin plugin) {
		...
		Arrays.asList(inventoryFiles).forEach(file -> {
			...
			PluginBedWars.getInstance().getServer().getConsoleSender().sendMessage("Loaded inventory configuration: '" + name + "'");
		});
	}
