...
public class NoMoveRunnable extends BukkitRunnable implements TimeProvider {
	...
	public NoMoveRunnable(SoupMapConfiguration config){
		...
		for(BadblockPlayer p : GameAPI.getAPI().getOnlinePlayers()){
			...
			for(PlayerKit toUnlock : PluginSoup.getInstance().getKits().values()){
				if(!toUnlock.isVIP()){
			...
			if(good && !p.getPlayerData().getAchievementState(SPAchievementList.SG_ALLKITS).isSucceeds()){
				p.getPlayerData().getAchievementState(SPAchievementList.SG_ALLKITS).succeed();
				SPAchievementList.SG_ALLKITS.reward(p);
			}
			...
			if (!PluginSoup.getInstance().getMapConfiguration().isWithTeam()) {
				p.sendTranslatedTitle("survival.title_withteam");
				p.sendTimings(2, 20*5, 2);
			}
		}

		BukkitUtils.teleportPlayersToLocations(PluginSoup.getInstance().getMapConfiguration().getLocations(), null, player -> {
			return true;
		});
	...
	public void run() {
		...
		if( (time % 10 == 0 || time <= 5) && time > 0 && time <= 30){
			...
			for(Player player : Bukkit.getOnlinePlayers()){
				...
				bPlayer.sendTranslatedTitle(title.getKey(), title.getObjects());
				bPlayer.sendTimings(2, 30, 2);
				bPlayer.sendTranslatedMessage("survival.scoreboard.teams_message_" + PluginSoup.getInstance().getMapConfiguration().isWithTeam());
			}
