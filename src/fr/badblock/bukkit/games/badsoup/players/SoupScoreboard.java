...
public class SoupScoreboard extends BadblockScoreboardGenerator {
	...
	public SoupScoreboard(BadblockPlayer player){
	...
	public void generate(){
		...
		if (PluginSoup.getInstance().getMapConfiguration() != null) {
			objective.changeLine(i,  i18n("survival.scoreboard.teams_" + PluginSoup.getInstance().getMapConfiguration().isWithTeam())); i--;
		}
	...
	private int alivePlayers(){
		return (int) GameAPI.getAPI().getRealOnlinePlayers().stream().filter(player -> !player.inGameData(SoupData.class).death).count();
	}
