...
public class ZombieListener extends BadListener {
	...
	public void onDamage(EntityDamageByEntityEvent e){
		...
		boolean damagerZombie = damager.inGameData(SoupData.class).zombie;
		boolean damagedZombie = damaged.inGameData(SoupData.class).zombie;
	...
	public void onDisconnect(PlayerQuitEvent e){
		boolean must = GameAPI.getAPI().getOnlinePlayers().stream().filter(player -> player.inGameData(SoupData.class).zombie && player != e.getPlayer()).count() == 0;
	...
	public void onMove(PlayerMoveEvent e){
		if(!isIn(e.getFrom()) && isIn(e.getTo())){
			...
			if(p.inGameData(SoupData.class).zombie)
				return;
			
			if(GameAPI.getAPI().getOnlinePlayers().stream().filter(player -> player.inGameData(SoupData.class).zombie).count() > 0){
				asPlayer(p);
	...
	protected void zombify(BadblockPlayer p){
		...
		p.inGameData(SoupData.class).zombie = true;
	...
	protected boolean isIn(Location loc){
		return PluginSoup.getInstance().getConfiguration().zombieGame.getHandle().isInSelection(loc);
	}
}
